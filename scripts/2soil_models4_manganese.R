...
soil1 <- soil %>% 
  select(plot_id, sample_n, site, type, treatment, position, Mn) %>% 
...
f1a <- Mn ~ treatment + type + position + treatment*type + type*position + treatment*position

#Log transform data
f1b <- log(Mn) ~ treatment + type + position + treatment*type + type*position + treatment*position
...
residplot(m1.v1, group = "type", newwd=F) 
...
MuMIn::AICc(gls1, gls2, lme1, lme2) #log() considerably improves the models, lme1 has the best AICc
...
f1 <- Mn ~ treatment + type + position + treatment*type + type*position + treatment*position
f2 <- Mn ~ treatment + type
f3 <- Mn ~ treatment + position
f4 <- Mn ~ position + type
f5 <- Mn ~ treatment + type + position
f6 <- Mn ~ treatment + type + position + treatment*type
f7 <- Mn ~ treatment + type + treatment*type
f8 <- Mn ~ treatment + type + position + type*position
f9 <- Mn ~ type + position + type*position
f10 <- Mn ~ treatment + type + position + treatment*position
f11 <- Mn ~ treatment + position + treatment*position
f12 <- Mn ~ treatment + type + position + treatment*type + type*position
f13 <- Mn ~ treatment + type + position + treatment*type + treatment*position
f14 <- Mn ~ treatment + type + position + treatment*position + treatment*type
...
#Run model selection
sel_manganese <- model.sel(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14)
#Look at the models in the table.
#Average the ones with delta <2, if just one, that is the best model
#m4, m3, m2, m9 are the best models

m_avgd <- model.avg(sel_manganese, subset = delta <2)
summary(m_avgd)
...
#Final model ####
#Re-run by REML
m_manganese <- lme(Mn ~ position + treatment + type + position*type,
                   random = ~1|site,
                   method = "REML",
                   data = soil1)
summary(m_manganese)

residplot(m_manganese, newwd = F) #residuals OK but a bit strange
acf(residuals(m_manganese)) # no autocorrelation


#Plot####
plot_data_mn <- m_manganese %>%
  tidy_plus_plus(tidy_fun = broom.helpers::tidy_parameters,
                 effects = "fixed",
  ...
  add_row(var_label = "Exclusion treatment", label = "Insect suppression", estimate = 1, 
          std.error = NA, conf.low = NA, conf.high = NA, statistic = NA, p.value = NA,
          .after = 2)

plot_data_mn <- plot_data_mn %>%
    mutate(
...
ggsave("figures/soil/models/m4_manganese.png", plot_manganese,
       width = 8,
       height = 6)

#Model results####
model_est <- m_manganese %>% tidy_plus_plus(
...
writexl::write_xlsx(model_est, "results/soil/m_manganese.xlsx")

#Estimated model means####
model.rg <- update(ref_grid(m_manganese), tran = "log") #to backtransform from log
emmeans_manganese <- as.data.frame(emmeans(model.rg, ~ position | type | treatment, 
                                          type = "response"
)) %>% 
...
writexl::write_xlsx(emmeans_manganese, "results/soil/emmeans_m_manganese.xlsx")

emmeans_manganese_plot <- emmeans_manganese %>% 
  ...
  geom_text(label = emmeans_manganese$response,
            nudge_x = 0.2,
            size = 3)

emmeans_manganese_plot 

ggsave("figures/soil/models/emmeans_manganese.png", emmeans_manganese_plot,
       width = 8,
       height = 4)