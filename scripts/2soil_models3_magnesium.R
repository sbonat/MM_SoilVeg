...
soil1 <- soil %>% 
  select(plot_id, sample_n, site, type, treatment, position, Mg) %>% 
...
f1a <- Mg ~ treatment + type + position + treatment*type + type*position + treatment*position

#Log transform data
f1b <- log(Mg) ~ treatment + type + position + treatment*type + type*position + treatment*position
...
MuMIn::AICc(m1.v2, gls1, gls2, lme1, lme2) #log() considerably improves the models, gls1, gls2 and lme1 have the best AICc
...
f1 <- Mg ~ treatment + type + position + treatment*type + type*position + treatment*position
...
f1 <- Mg ~ treatment + type + position + treatment*type + type*position + treatment*position
f2 <- Mg ~ treatment + type
f3 <- Mg ~ treatment + position
f4 <- Mg ~ position + type
f5 <- Mg ~ treatment + type + position
f6 <- Mg ~ treatment + type + position + treatment*type
f7 <- Mg ~ treatment + type + treatment*type
f8 <- Mg ~ treatment + type + position + type*position
f9 <- Mg ~ type + position + type*position
f10 <- Mg ~ treatment + type + position + treatment*position
f11 <- Mg ~ treatment + position + treatment*position
f12 <- Mg ~ treatment + type + position + treatment*type + type*position
f13 <- Mg ~ treatment + type + position + treatment*type + treatment*position
f14 <- Mg ~ treatment + type + position + treatment*position + treatment*type
...
#Run model selection
sel_magnesium <- model.sel(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14)
#Look at the models in the table.
#Average the ones with delta <2, if just one, that is the best model
#m4 and m2 are the best models

m_avgd <- model.avg(m4, m2)
summary(m_avgd)
rm(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14, m_avgd)

#Final model ####
#Re-run by REML
m_magnesium <- lme(Mg ~ position + treatment + type,
                 random = ~1|site,
                 method = "REML",
                 data = soil1)
summary(m_magnesium)
residplot(m_magnesium, newwd = F) #residuals OK
acf(residuals(m_magnesium)) # no autocorrelation


#Plot####

plot_data_mg <- m_magnesium %>% 
...
ggsave("figures/soil/models/m3_magnesium.png", plot_magnesium,
       width = 8,
       height = 6)

#Model results####
model_est <- m_magnesium %>% tidy_plus_plus(
...
writexl::write_xlsx(model_est, "results/soil/m_magnesium.xlsx")

#Estimated model means####
model.rg <- update(ref_grid(m_magnesium), tran = "log") #to backtransform from log
emmeans_magnesium <- as.data.frame(emmeans(model.rg, ~ position | type | treatment, 
                                          type = "response"
)) %>% 
...
writexl::write_xlsx(emmeans_magnesium, "results/soil/emmeans_m_magnesium.xlsx")

emmeans_magnesium_plot <- emmeans_magnesium %>% 
  ...
  labs(x = "",
       ...
       title = "Magnesium")+
  geom_errorbar(aes(ymin = upper.SE, ymax = lower.SE), width = 0.2)+
  ...
  geom_text(label = emmeans_magnesium$response,
            nudge_x = 0.2,
            size = 3)

emmeans_magnesium_plot

ggsave("figures/soil/models/emmeans_magnesium.png", emmeans_magnesium_plot,
       width = 8,
       height = 4)