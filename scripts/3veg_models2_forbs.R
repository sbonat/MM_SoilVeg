...
#Plot####
plot_data_forb <- m_forb %>% tidy_plus_plus(
...
plot_forb <- plot_data_forb %>% 
  ...
  labs(x = "",
       y = "Estimate",
       title = "Model estimates for forb (Post-Baseline)")+
...
model_est <- m_forb %>% tidy_plus_plus(
  exponentiate = F, #exponentiate as the var was log transformed to deal with heterogeneity
